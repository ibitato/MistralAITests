...
def calculate_expression(expression: str) -> dict:
    ...
    try:
        ...
        if not all(c in allowed_chars for c in expression):
            return {"error": "Expression contains invalid characters", "success": False}
...
def web_search(query: str, max_results: int = 3) -> dict:
    ...
    if not results:
        results = [
            ...
            "Try a different search query related to weather, geography, or programming",
        ]
...
def main() -> None:
    ...
    messages = [
        {
            "role": "user",
            "content": "I need to plan a trip to Paris. Can you check the weather there and calculate the total cost if I spend $150 per day for 5 days?",
        }
    ]
    ...
    try:
        ...
        if result["tool_execution_history"]:
            ...
            for i, tool_exec in enumerate(result["tool_execution_history"], 1):
                print(
                    f"   {i}. {tool_exec['function']} - {'✅ Success' if tool_exec['success'] else '❌ Failed'}"
                )
    ...
    messages = [{"role": "user", "content": "What's the weather on Mars?"}]
    ...
    messages = [{"role": "user", "content": "Calculate 25 + 35"}]
    ...
    print(
        "   • Mistral AI Tool Calling Guide: https://docs.mistral.ai/api/tool-calling"
    )
