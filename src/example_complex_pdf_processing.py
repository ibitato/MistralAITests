...
def main() -> None:
    ...
    try:
        ...
        if "error" in structure:
            ...
            print(
                f"   • Table of Contents: {'✅' if doc_struct['has_table_of_contents'] else '❌'}"
            )
            print(f"   • Bookmarks: {'✅' if doc_struct['has_bookmarks'] else '❌'}")
            print(f"   • Hyperlinks: {'✅' if doc_struct['has_hyperlinks'] else '❌'}")
            print(
                f"   • Annotations: {'✅' if doc_struct['has_annotations'] else '❌'}"
            )
    ...
    try:
        ...
        if "error" in structured_result:
            ...
            print(
                f"   • Revenue: {fin['revenue']['2024']} ({fin['revenue']['growth']} growth)"
            )
            print(
                f"   • Profit: {fin['profit']['2024']} ({fin['profit']['growth']} growth)"
            )
            print(
                f"   • Expenses: {fin['expenses']['2024']} ({fin['expenses']['growth']} growth)"
            )
    ...
    try:
        ...
        if "error" in analysis:
            ...
            print(
                f"   • Avg Sentence Length: {content_stats['average_sentence_length']:.1f} words"
            )
            ...
            print(
                f"   • Positive Sections: {', '.join(sentiment['positive_sections'])}"
            )
            print(
                f"   • Negative Sections: {', '.join(sentiment['negative_sections'])}"
            )
    ...
    logger.info(
        f"Complex PDF processing example completed in {elapsed_time:.2f} seconds"
    )
